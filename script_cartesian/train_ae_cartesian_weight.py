...
def train_model(model, train_loader, val_loader, optimizer, num_epochs, patience, weight):
    train_losses = []
    val_losses = []
    ...
    for epoch in range(num_epochs):
        ...
        for (hypotheses, aggr_hypothesis), ground_truth in train_loader:
            ...
            # Loss for individual hypotheses
            individual_losses = 0.0
            # batch_size = hypotheses.size(0)  # 64
            num_hypotheses = hypotheses.size(1)  # 20

            for i in range(num_hypotheses):  # Loop through each hypothesis in the 20 hypotheses
                _hyp = hypotheses[:, i, :]  # Select the i-th hypothesis for all 64 samples
                _rec_hyp = model(_hyp)

                # Compute the loss for this hypothesis
                individual_losses += loss_function(_rec_hyp, _gt)

            # Average over all hypotheses (20)
            individual_losses /= num_hypotheses

            # Combine mean loss and weighted individual loss
            total_loss = mean_loss + weight * individual_losses
            total_loss.backward()
            optimizer.step()
...
if __name__ == '__main__':
    ...
    weight = 0.1
    num_epochs = 100
    train_losses, val_losses = train_model(model, train_loader, val_loader, optimizer, num_epochs, 3, weight)
