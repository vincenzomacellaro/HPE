...
if __name__ == '__main__':
    ...
    for i in tqdm(range(0, len(ground_truth), batch_size), total=total_batches, desc="Processing", unit="batch"):
        ...
        for batch_idx in range(len(gt_batch)):
            ...
            hyp_pp = preprocess_batch(hyp_tensor)
            vae_input = hyp_pp[:, :-16]
            ...
            rec_input = np.concatenate((_rec_pose_np, hyp_pp[:, -16:]), axis=1)
            aggr_hyp = np.median(rec_input, axis=0, keepdims=True)  # (1, 17, 3)

            rec_pp = postprocess_batch(torch.from_numpy(aggr_hyp))

            # mm -> m
            predicted = rec_pp.unsqueeze(0).unsqueeze(0).unsqueeze(0) * 1000
